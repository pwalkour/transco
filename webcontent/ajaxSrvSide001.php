...
function connect_db()
{
	...
	$request= "call slaGetHistoData(".$effdt.");";
	...
	if ($couter_check==0) {
		$slaDataElement = $slaXmlRoot->appendChild($slaXmlDoc->createElement("sla"));
		$slaDataElement->appendChild($slaXmlDoc->createElement("id_sla",0));
		$slaDataElement->appendChild($slaXmlDoc->createElement("effdt","1900-01-01"));
		$slaDataElement->appendChild($slaXmlDoc->createElement("serial",0));
		$slaDataElement->appendChild($slaXmlDoc->createElement("is_used",0));
		$slaDataElement->appendChild($slaXmlDoc->createElement("Pas de donnée",""));
		$slaDataElement->appendChild($slaXmlDoc->createElement("No data",""));
	}
